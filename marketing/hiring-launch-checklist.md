...
- [ ] Submit one real test application using a test résumé that is a PDF, DOC, or DOCX and no larger than 5 MB.
- [ ] Confirm the submission creates a dashboard record, sends the configured notification, preserves the attachment, and reaches `/application-received`.
- [ ] Remove the test submission and test notification artifacts according to the company's retention process.
- [ ] Restrict form-dashboard access to the hiring team and confirm no résumé data is handled through public comments or direct messages.
